...
class DecayMaskedLinear(nn.Linear):
    def __init__(self, *args, **kwargs):
        ...
        mask1 = torch.triu(mask1, 1)
        self.register_buffer('mask0', mask0)
        self.register_buffer('mask1', mask1)

        gain = nn.init.calculate_gain('relu')
        std = gain * math.sqrt(3.0 / self.weight.size(1))
...
def main():
    '''
        python train_MaskPSN_L1loss.py -data-dir ./datasets/CIFAR10 -amp -opt sgd -channels 128 -epochs 256 -P 32  -device cuda:1
    '''
    ...
    out_dir = f'secf10_T_{args.T}P_{args.P}MaskedPSN_l1loss_0.06_e{args.epochs}_b{args.b}_{args.opt}_lr{args.lr}_c{args.channels}'
            ...
            with torch.cuda.amp.autocast(enabled=scaler is not None):
                ...
                for m in net.modules():
                    if isinstance(m, (MaskedPSN)):
                        loss += (0.06 * m.fc.weight * m.fc.mask1).abs().sum()
