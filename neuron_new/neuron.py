...
class LinearNode(BaseNode):
    ...
    def extra_repr(self):
        return f'a={self.a}, b={self.b}, learnable={self.learnable} v_threshold={self.v_threshold}, v_reset={self.v_reset}, detach_reset={self.detach_reset}, step_mode={self.step_mode}, backend={self.backend}'
...
if __name__ == '__main__':
    ...
    with torch.cuda.device(device):
        # for surrogate in surrogate._has_cuda_:
            # print(surrogate)
            SJ_IFNode = LinearNode(a=1.0, b=1.0, learnable=True, backend='torch', surrogate_function=surrogate.ATan(), step_mode='m', detach_reset=True)
            My_IFNode = IFNode(backend='triton', surrogate_function=surrogate.Sigmoid(), step_mode='s', detach_reset=True)
