...
class DecayMaskedLinear(nn.Linear):
    ...
    def gen_masked_weight(weight: torch.Tensor, k: float, mask0: torch.Tensor, mask1: torch.Tensor):
        return weight.tanh() * (k * mask0 + (1. - k) * mask1)
...
def main():
    '''
        python train_maskPSN_tanh_all.py -data-dir ./datasets/CIFAR10 -amp -opt sgd -channels 128 -epochs 256 -P 32  -device cuda:2
    '''
    ...
    out_dir = f'secf10_T_{args.T}P_{args.P}MaskedPSN_all_tanh_x^(1/4)_e{args.epochs}_b{args.b}_{args.opt}_lr{args.lr}_c{args.channels}'
