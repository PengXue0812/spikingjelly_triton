...
class CIFAR100Net(nn.Module):
    def __init__(self, channels, T, l1_coeff):
        super().__init__()
        conv = []
        for i in range(2):
            for j in range(3):
    ...
    def forward(self, x_seq: torch.Tensor):
        y_seq = self.conv(x_seq)
        y_seq = self.fc(y_seq)
        
        if self.training:
            l1_loss = torch.tensor(0.0, device=x_seq.device) 

            for module in self.modules():
                if isinstance(module, neuron.PSN):
                    l1_loss += module.weight.abs().sum()

            return y_seq, self.l1_coeff * l1_loss  
        else:
            return y_seq
...
def main():
    ...
    '''
    python train_PSN_L1Loss_CIFAR100.py -data-dir ./datasets/CIFAR100 -amp -opt sgd -channels 128 -epochs 128 -l1_coeff 0.0001 
    ...
    parser.add_argument('-l1_coeff', default=0.0001, type=float)
    ...
    out_dir = f'CIFAR100_PSN_L1Loss_T_{args.T}_e{args.epochs}_b{args.b}_{args.opt}_lr{args.lr}_c{args.channels}_l1_coeff{args.l1_coeff}'
    if args.amp:
        out_dir += '_amp'
    ...
    net = CIFAR100Net(args.channels, args.T, args.l1_coeff)
    net.to(args.device)
