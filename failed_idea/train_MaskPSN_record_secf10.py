...
def main():
    ...
    out_dir = f'secf10_T_{args.T}P_{args.P}MaskedPSN_record_e{args.epochs}_b{args.b}_{args.opt}_lr{args.lr}_c{args.channels}'
    if args.amp:
        out_dir += '_amp'
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for m in net.modules():
            if isinstance(m, (MaskedPSN)):
                weight[id] = m.fc.weight.detach().cpu().numpy()
                id += 1

        weight_checkpoint[epoch] = weight
        lambda_ = min(epoch / (args.epochs - 1) * 8, 1.)
        weight_checkpoint[epoch]['lambda_'] = lambda_

        if lambda_ == 1.:
            # 保存weight_checkpoint
            torch.save(weight_checkpoint, os.path.join(pt_dir, 'weight_checkpoint.pth'))
            #   退出
            break
